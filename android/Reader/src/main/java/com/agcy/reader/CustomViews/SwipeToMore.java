 ...
 */
public final class SwipeToMore implements View.OnTouchListener {
    ...
    private List<Action> mToDoActions;
    ...
    /**
     * Defines the mode a {@link com.agcy.reader.CustomViews.SwipeToMore} handles multiple undos.
     */
     ...
     */
    public SwipeToMore(AbsListView listView, OnActionCallback callback) {
     ...
     */
    public SwipeToMore(AbsListView listView, OnActionCallback callback, UndoMode mode) {
        ...
        switch(mode) {
            case SINGLE_UNDO:
                mToDoActions = new ArrayList<Action>(1);
                break;
            default:
                mToDoActions = new ArrayList<Action>(10);
                break;
        }
    ...
    private void changePopupText() {
        String msg = "";
        if(mToDoActions.size() > 1 && mDeleteMultipleString != null) {
            msg = String.format(mDeleteMultipleString, mToDoActions.size());
        } else if(mToDoActions.size() >= 1) {
            // Set title from single undoable or when no multiple deletion string
            // is given
            if(mToDoActions.get(mToDoActions.size() - 1).getTitle() != null) {
                msg = mToDoActions.get(mToDoActions.size() - 1).getTitle();
            } else {
                msg = mDeleteString;
            }
        }
